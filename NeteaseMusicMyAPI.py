...
from myobject.diyTools import catchErrors
...
var CryptoJS = CryptoJS || function (u, p) {
    var d = {}, l = d.lib = {}, s = function () {
        ...
        reset: function () {
            ...
            this.j7c.concat(a);
            this.bAy3x += a.sigBytes
        }, CE7x: function (a) {
            var c = this.j7c, e = c.words, j = c.sigBytes, k = this.blockSize, b = j / (4 * k),
                b = a ? u.ceil(b) : u.max((b | 0) - this.bAw3x, 0);
            a = b * k;
            j = u.min(4 * a, j);
            if (a) {
                for (var q = 0; q < a; q += k) this.bAB3x(e, q);
                q = e.splice(0, a);
                c.sigBytes -= j
            }
...
(function (u) {
    ...
    r = r.MD5 = v.extend({
        bkO9F: function () {
            this.dP9G = new w.init([1732584193, 4023233417, 2562383102, 271733878])
        }, bAB3x: function (q, n) {
            for (var a = 0; 16 > a; a++) {
                var c = n + a, e = q[c];
                q[c] = (e << 8 | e >>> 24) & 16711935 | (e << 24 | e >>> 8) & 4278255360
            }
            var a = this.dP9G.words, c = q[n + 0], e = q[n + 1], j = q[n + 2], k = q[n + 3], z = q[n + 4], r = q[n + 5],
            ...
            var b = this.j7c, n = b.words, a = 8 * this.bAy3x, c = 8 * b.sigBytes;
            n[c >>> 5] |= 128 << 24 - c % 32;
...
CryptoJS.lib.Cipher || function (u) {
    var p = CryptoJS, d = p.lib, l = d.Base, s = d.WordArray, t = d.BufferedBlockAlgorithm, r = p.enc.Base64,
        w = p.algo.EvpKDF, v = d.Cipher = t.extend({
            cfg: l.extend(), createEncryptor: function (e, a) {
                return this.create(this.bkT9K, e, a)
            }, createDecryptor: function (e, a) {
                return this.create(this.cAH0x, e, a)
            }, init: function (e, a, b) {
                ...
                e && this.YK5P(e);
                return this.YE5J()
            }, keySize: 4, ivSize: 4, bkT9K: 1, cAH0x: 2, bkN9E: function (e) {
                return {
    ...
    q.Encryptor = q.extend({
        processBlock: function (e, a) {
            var b = this.bBe3x, c = b.blockSize;
            x.call(this, e, a, c);
            b.encryptBlock(e, a);
            this.bAZ3x = e.slice(a, a + c)
        }
    });
    q.Decryptor = q.extend({
        processBlock: function (e, a) {
            var b = this.bBe3x, c = b.blockSize, d = e.slice(a, a + c);
            b.decryptBlock(e, a);
            x.call(this, e, a, c);
            this.bAZ3x = d
        }
    ...
    d.BlockCipher = v.extend({
        cfg: v.cfg.extend({mode: b, padding: q}), reset: function () {
            ...
            if (this.bAV3x == this.bkT9K) var c = a.createEncryptor; else c = a.createDecryptor, this.bAw3x = 1;
            this.fg9X = c.call(a, this, b && b.words)
        }, bAB3x: function (a, b) {
            this.fg9X.processBlock(a, b)
        }, YE5J: function () {
            var a = this.cfg.padding;
            if (this.bAV3x == this.bkT9K) {
                a.pad(this.j7c, this.blockSize);
                var b = this.CE7x(!0)
            } else b = this.CE7x(!0), a.unpad(b);
            return b
        }, blockSize: 4
    });
...
(function () {
    ...
    var H = [0, 1, 2, 4, 8, 16, 32, 64, 128, 27, 54], d = d.AES = p.extend({
        bkO9F: function () {
            for (var a = this.J7C, c = a.words, d = a.sigBytes / 4, a = 4 * ((this.cAF0x = d + 6) + 1), e = this.cAE0x = [], j = 0; j < a; j++) if (j < d) e[j] = c[j]; else {
                var k = e[j - 1];
            ...
            this.bBg3x(a, b, this.cAE0x, t, r, w, v, l)
        }, decryptBlock: function (a, c) {
            var d = a[c + 1];
            a[c + 1] = a[c + 3];
            a[c + 3] = d;
            this.bBg3x(a, c, this.cAD0x, b, x, q, n, s);
            d = a[c + 1];
            a[c + 1] = a[c + 3];
            a[c + 3] = d
        }, bBg3x: function (a, b, c, d, e, j, l, f) {
            for (var m = this.cAF0x, g = a[b] ^ c[0], h = a[b + 1] ^ c[1], k = a[b + 2] ^ c[2], n = a[b + 3] ^ c[3], p = 4, r = 1; r < m; r++) var q = d[g >>> 24] ^ e[h >>> 16 & 255] ^ j[k >>> 8 & 255] ^ l[n & 255] ^ c[p++], s = d[h >>> 24] ^ e[k >>> 16 & 255] ^ j[n >>> 8 & 255] ^ l[g & 255] ^ c[p++], t = d[k >>> 24] ^ e[n >>> 16 & 255] ^ j[g >>> 8 & 255] ^ l[h & 255] ^ c[p++], n = d[n >>> 24] ^ e[g >>> 16 & 255] ^ j[h >>> 8 & 255] ^ l[k & 255] ^ c[p++], g = q, h = s, k = t;
...
class NMMA:
    ...
    # 初始化
    def __init__(self):
        self.Js = execjs.compile(jsoncode)
        self.headers = {
            'Accept': '*/*',
            ...
            'Cookie': '_ntes_nnid = 252723d70bfcb15a839af67a98a1dc1a,1522936569400;_ntes_nuid = 252723d70bfcb15a839af67a98a1dc1a;usertrack= ezq0plrJrsxL1qd0B3F2Ag==;_ga=GA1.2.1714224760.1523166929;_gid=GA1.2.1638922464.1523166929'
        }
        self.searchType = {'单曲': 1, '专辑': 10, '歌手': 100, '歌单': 1000, '视频': 1014}
        # self.ipProxyPool=IpPool()

    # 参数加密
    def __paramsEncrypt(self, jsonKey):
        param1 = json.dumps(jsonKey)
        param2 = '010001'
        ...
        obj = self.Js.call('d', param1, param2, param3, param4)
        return obj['encText'], obj['encSecKey']

    # 发送搜索请求
    @catchErrors
    def __requestInternet(self, list, url,headers=None):
        if headers is None:
            headers=self.headers
        params, encSecKey = self.__paramsEncrypt(list)
        data = {
            'csrf_token': '',
            'params': params,
            'encSecKey': encSecKey
    ...
    # 搜索信息
    @catchErrors
    def Search(self, keyword, limit, page, type):
        if not type in self.searchType.keys():
            return Exception('搜索类型错误')
        url = r'http://music.163.com/weapi/cloudsearch/get/web'
        list = {'s': keyword, 'type': self.searchType[type], 'offset': page * 30, 'limit': limit, 'csrf_token': ''}
        return self.__requestInternet(list=list, url=url)

    #获取歌单歌曲列表

    def getListSongs(self,id):
        url='http://music.163.com/playlist?id='+id
        data={'id':id}
        res=requests.get(url=url,data=data,headers=self.headers)
        html=res.text
        bs=BeautifulSoup(html,"lxml")
        idTags=bs.find_all('a',href=re.compile('^/song\?id='))
        songsIdList=[idtag["href"][6:] for idtag in idTags]
        songsNameList=[idtag.string for idtag in idTags]
        return songsIdList,songsNameList

    # 获取单曲信息
    @catchErrors
    def getSongDetail(self, id):
        url = r'http://music.163.com/weapi/v3/song/detail'
        list = {"id": id, "c": r'[{"id":"%s"}]' % id, "csrf_token": ""}
        return self.__requestInternet(list=list, url=url)

    # 获取歌曲链接
    @catchErrors
    def getSongUrl(self, id):
        url = r'http://music.163.com/weapi/song/enhance/player/url'
        list = {'ids': "[%s]" % id, 'br': 128000, 'csrf_token': ""}
        req = self.__requestInternet(list=list, url=url)
        return req['data'][0]['url']

    # 进行歌曲下载任务
    @catchErrors
    def downloadSong(self, url, filePath):
        headers = self.headers
    ...
    #获取视频信息
    @catchErrors
    def getVideoDetail(self,id):
        url = r'http://music.163.com/weapi/cloudvideo/v1/video/detail'
        list = {"id": id, "csrf_token": ""}
        headers=self.headers
        headers['Referer']='http://music.163.com/video?id=%s'%id
        return self.__requestInternet(list=list, url=url,headers=headers)

    # 获取视频链接
    @catchErrors
    def getVideoUrl(self,id,br):
        url = r'http://music.163.com/weapi/cloudvideo/playurl'
        list = {"ids": '["%s"]' % id, "resolution":br,"csrf_token":""}
        headers = self.headers
        headers['Referer'] = 'http://music.163.com/video?id=%s' % id
        req = self.__requestInternet(list=list, url=url,headers=headers)

        return req['urls'][0]['url']

    #进行视频下载任务
    @catchErrors
    def downloadVideo(self, id,url, filePath):
        headers = self.headers
        httpadd=re.findall('http://[a-zA-Z0-9\.]+126\.net/',url)[0]
        headers['Host'] = httpadd
        headers['Referer'] = 'http://music.163.com/video?id=%s' % id
    ...
    #获取MV信息
    @catchErrors
    def getMVDetail(self,id):
        url = r'http://music.163.com/weapi/v1/mv/detail'
        list = {"type":"MP4","id": id, "csrf_token": ""}
        headers=self.headers
        headers['Referer']='http://music.163.com/mv?id=%s'%id
        return self.__requestInternet(list=list, url=url,headers=headers)

    # 获取MV链接
    @catchErrors
    def getMVUrl(self,id,br):
        url = r'http://music.163.com/weapi/song/enhance/play/mv/url'
        list = {"id": "%s" % id, "r":br,"csrf_token":""}
        headers = self.headers
        headers['Referer'] = 'http://music.163.com/mv?id=%s' % id
        req = self.__requestInternet(list=list, url=url,headers=headers)
        return req['data']['url']
    ...
    #进行MV下载任务
    @catchErrors
    def downloadMV(self, id,url, filePath):
        headers = self.headers
        httpadd=re.findall('http://[a-zA-Z0-9\.]+126\.net/',url)[0]
        headers['Host'] = httpadd
        headers['Referer'] = 'http://music.163.com/mv?id=%s' % id
...
class NetMusic:
    def __init__(self):
        self.mp3Process = NMMA()

    @catchErrors
    def searchSongsInfo(self, keyword, limit=30, page=0):
        result = self.mp3Process.Search(keyword=keyword, type='单曲', limit=limit, page=page)
        if result is None or result['code']!=200:
            Exception('服务器信息获取失败')
        songscount = result['result']['songCount']
        songInfos = [{
            'name': songInfo['name'],
            'id': songInfo['id'],
            'singer': ','.join([s['name'] for s in songInfo['ar']]),
            'album': songInfo['al']['name'],
            'MV': songInfo['mv']
        } for songInfo in result['result']['songs']]
        return songInfos

    @catchErrors
    def searchSingersInfo(self, keyword, limit=90, page=0):
        result = self.mp3Process.Search(keyword=keyword, type='歌手', limit=limit, page=page)
        if result is None or result['code'] != 200:
            Exception('服务器信息获取失败')
        artistcount = result['result']['artistCount']
        artistInfos = [{
            'name': artistInfo['name'],
            'id': artistInfo['id'],
            'albumSize': artistInfo['albumSize'],
            'MVSize': artistInfo['mvSize']
        } for artistInfo in result['result']['artists']]
        return artistInfos

    @catchErrors
    def searchAlbumsInfo(self, keyword, limit=75, page=0):
        result = self.mp3Process.Search(keyword=keyword, type='专辑', limit=limit, page=page)
        if result is None or result['code']!=200:
            Exception('服务器信息获取失败')
        albumscount = result['result']['songCount']
        albumInfos = [{
            'name': albumInfo['name'],
            'id': albumInfo['id'],
            'artist': ','.join([s['name'] for s in albumInfo['artists']]),
            'size': albumInfo['size'],
        } for albumInfo in result['result']['albums']]
        return albumInfos

    @catchErrors
    def searchVideosInfo(self, keyword, limit=20, page=0):
        result = self.mp3Process.Search(keyword=keyword, type='视频', limit=limit, page=page)
        if result is None or result['code']!=200:
            Exception('服务器信息获取失败')
        videocount = result['result']['videoCount']
        videoInfos = [{
            'title': videoInfo['title'],
            'id': videoInfo['vid'],
            'creator': videoInfo['creator'][0]['userName']
        } for videoInfo in result['result']['videos']]
        return videoInfos

    @catchErrors
    def searchListsInfo(self, keyword, limit=75, page=0):
        result = self.mp3Process.Search(keyword=keyword, type='歌单', limit=limit, page=page)
        if result is None or result['code']!=200:
            Exception('服务器信息获取失败')
        ...
        playlists = [{
            'name': listInfo['name'],
            'id': listInfo['id'],
            'creator':listInfo['creator']['nickname'],
            'tags':listInfo['creator']['expertTags']
        } for listInfo in result['result']['playlists']]
        return playlists

    @catchErrors
    def getSongDetailById(self, id):
        result = self.mp3Process.getSongDetail(id=id)
        if result is None or result['code']!=200:
            Exception('服务器信息获取失败')
    ...
    @catchErrors
    def getVideoDetailById(self, id):
        result = self.mp3Process.getVideoDetail(id=id)
        if result is None or result['code']!=200:
            Exception('服务器信息获取失败')
        result = result["data"]
        videoDetail = {
    ...
    @catchErrors
    def getMVDetailById(self, id):
        result = self.mp3Process.getMVDetail(id=id)
        result = result["data"]
        mvDetail = {
            'title': result['name'],
            'id': result['id'],
            'resolutions': result['brs'],
            'artist': result['artistName']
        }
        return mvDetail

    @catchErrors
    def showListBasicInfo(self,id,jsonlist):
        for listInfo in jsonlist:
            if listInfo['id'] == id:
                print(listInfo)

    @catchErrors
    def getSongsInListByListId(self, id):
        songsIdList, songsNameList = self.mp3Process.getListSongs(id=id)
        if len(songsIdList)==len(songsNameList):
            songItems=[(songsIdList[i],songsNameList[i]) for i in range(len(songsIdList))]
            return songItems
        else:
            return []

    @catchErrors
    def downloadSong(self, id, filePath):
        details = self.getSongDetailById(id=id)
        singers =[singer['name'] for singer in details['singer']]

        url = self.mp3Process.getSongUrl(id=id)
        path = filePath + "/" + ','.join(singers) + " - " + details['name'] + ".mp3"
        self.mp3Process.downloadSong(url=url, filePath=path)

        mfile = MP3(path, ID3=EasyID3)
        mfile['title'] = [details['name']]
        mfile['artist'] = ['/'.join(singers)]
        mfile['album'] = [details['album']['name']]
        mfile['tracknumber'] = ['99']
        mfile.save()
        return 'SongDownloadFinish'

    @catchErrors
    def downloadVideo(self, id, filePath,br=240):
        details = self.getVideoDetailById(id=id)

        for r in details['resolutions']:
            pbr=int(r['resolution'])
            if pbr > br :
                br=pbr

        url = self.mp3Process.getVideoUrl(id=id,br=br)
        path = filePath + "/" + details['artist'] + " - " + details['title'] + ".mp4"
        self.mp3Process.downloadVideo(id=id,url=url, filePath=path)

        return 'VideoDownloadFinish'

    @catchErrors
    def downloadMV(self, id, filePath, br=240):
        details = self.getMVDetailById(id=id)
        ...
        url = self.mp3Process.getMVUrl(id=id, br=br)
        path = filePath + "/" + details['artist'] + " - " + details['title'] + ".mp4"
        self.mp3Process.downloadMV(id=id, url=url, filePath=path)

        return 'MVDownloadFinish'

    @catchErrors
    def console(self):
        while 1:
            type = input("请输入查询类型：单曲（可下载）、歌单（未完善）、专辑、歌手、视频（可下载），输入0退出：\t")
            if type =='0':
                break
            keyword = input("请输入查询关键字：\t")
            if keyword:
                if type == '单曲':
                    jsondata = self.searchSongsInfo(keyword)
                    for data in jsondata:
                        print(data)
                    ID = input('请输入ID:\t')
                    self.downloadSong(id=ID, filePath="D:/CloudMusic")
                elif type == '专辑':
                    jsondata = self.searchAlbumsInfo(keyword)
                    for data in jsondata:
                        print(data)
                elif type == '歌手':
                    jsondata = self.searchSingersInfo(keyword)
                    for data in jsondata:
                        print(data)
                elif type == '歌单':
                    jsondata = self.searchListsInfo(keyword)
                    for data in jsondata:
                        print(data)
                    ID = input('请输入ID:\t')
                    self.showListBasicInfo(id=ID, jsonlist=jsondata)
                    songsItems = self.getSongsInListByListId(id=ID)
                    for item in songsItems:
                        print(item)
                elif type == '视频':
                    jsondata = self.searchVideosInfo(keyword)
                    for data in jsondata:
                        print(data)
                    ID = input('请输入ID:\t')
                    self.downloadVideo(id=ID, filePath="D:/CloudMusic/MV")
            keyword = None
            type = None
        print("退出")
